...
bool Test3(FILE* fpOut)
{
	bool result = true;

	fprintf(fpOut, "==============================\n");
	fprintf(fpOut, "Test3: Beginning!\n");
	fprintf(fpOut, "==============================\n");
	fprintf(fpOut, "Memory Before the test:\n");
	MemCheck(fpOut,"Before Test3");
	MemStatsPrint(fpOut);
	fprintf(fpOut, "==============================\n");
	...
	{
		fprintf(fpOut, "Test3: The creation of the tree failed (%zd)\n", rc);
		result = false;
		exit(1);
	}
	...
	fprintf(fpOut, "Test3: Starting Read ...\n");

	if (fp == NULL)
	{
		fprintf(fpOut, "Test3: Could not open test data file!\n");
		return false;
	}
	...
	while (fgets(buffer, sizeof(buffer), fp) != NULL)
	{
		...
		if (debugWhat & DEBUG_INSERT)
			fprintf(fpDebug, "Test3: Inserting '%s'  %zd\n", buffer, count);
#endif
		...
		{
			BPPrintTree(fpOut, pTree);
			fprintf(fpOut, "Test3: Woops ... can't find '%s' which we just inserted!!!\n", buffer);
		...
		if (rc != BP_RC_Success)
		{
			fprintf(fpOut, "Test3: We had a problem: %zd\n", rc);
			return false;
		}
//			BPIntegrityCheck(fpOut, pTree);
	}
	...
	fprintf(fpOut, "Test3: Inserted %zd entries!\n", count);
	BPIntegrityCheck(fpOut, pTree);
