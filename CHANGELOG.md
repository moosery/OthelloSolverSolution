...
### Changed
- **`OthelloLevelEnumerator` / `OLECache`** — `OLELevelStatEntry` and `level_stats.json` now include four additional fields per level: `uniqueBoards` (deduped merge output count), `passBoards` (boards where current player had no legal moves, pass-folded inline), `endBoards` (terminal boards where neither player could move), and `maxMovesAny` (max children generated by any single board); all four are written after each completed level and preserved across restarts; existing cache entries without these fields read back as zero and are filled in on next run
- **`OthelloLevelEnumerator` / `OLEMain`** — version bumped to 0.4.15
