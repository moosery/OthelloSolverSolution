...
// One expanded child: canonical board + the move edge leading to it.
...
struct OLEGpuBuffers {
    BOARD*    d_accumA;     // accumulation buffer A (BOARD objects, 64 bytes/slot)
    BOARD*    d_accumB;     // accumulation buffer B
    ...
    uint8_t*  d_dupFlagsB;
    size_t    slotCapacity; // number of BOARD slots per buffer
};
