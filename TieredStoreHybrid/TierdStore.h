...
//   pArena             — optional caller-owned arena for the in-memory B+ tree.
//                        NULL (default) = use system malloc.
//                        Non-NULL = arena mode; caller must ensure the arena is large enough
//                        (>= maxMemoryBytes + node overhead) and must keep it valid until
